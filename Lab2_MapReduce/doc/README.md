...
Links to an external site.. (Note: this lab uses "coordinator" instead of the paper's "master".)
Getting started

Fetch the initial lab software with git
Links to an external site. (To learn more about git, look at the Pro Git book Links to an external site. or the git user's manual.)
...
If you run the test script now, it will hang because the coordinator never finishes. You can change `ret := false` to true in the Done function in mr/coordinator.go so that the coordinator exits immediately. Then: 
...
Ignore these messages; registering the coordinator as an RPC server

Links to an external site. checks if all its methods are suitable for RPCs (have 3 inputs); we know that Done is not called via RPC.
...
Hints
    ...
    Links to an external site. has some tips on developing and debugging.
 ...
 and to read such a file back: 
  ...
  for {
    ...
    When passing a pointer to a reply struct to the RPC system, the object that *reply points to should be zero-allocated. The code for RPC calls should always look like this:
...
Your program now runs on a single machine. Now, distribute your program to run on multiple machines (we recommend following the MapReduce paper

Links to an external site.). Your choice on the cloud, lab computers, or your own machines. You are not allowed to use a shared file system such as EFS.
...
Prepare to present this in the lab, with at least two workers, one coordinator, and a user, where you can see them all at once and kill some of them at will. The tests should pass for the basic part, but does not have to be integrated with the advanced part.
